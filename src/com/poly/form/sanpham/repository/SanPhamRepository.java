...
public class SanPhamRepository {
    ...
    public boolean isExistTenSanPham(String ten) {
        String query = "select TenSanPham from SanPham Where TenSanPham = ?";
        if (cn != null) {
            try {
                PreparedStatement ps = cn.prepareStatement(query);
                ps.setString(1, ten);
    ...
    public boolean isExistTenSanPhamWhenUpdate(String ten, Long id) {
        String query = "select TenSanPham from SanPham Where TenSanPham = ? AND IDSanPham NOT IN (?)";
        if (cn != null) {
            try {
                PreparedStatement ps = cn.prepareStatement(query);
                ps.setString(1, ten);
