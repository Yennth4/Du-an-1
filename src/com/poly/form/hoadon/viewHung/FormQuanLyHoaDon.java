...
//    HoaDonChiTietRepository hoaDonChiTietRepository = new HoaDonChiTietRepository();
        ...
        try {
            ...
            if (hoaDon.getTrangThai() == 1) {
                ...
                if (confirm == JOptionPane.YES_OPTION) {
                    JFileChooser avatarChooser = new JFileChooser("F:\\");
    ...
    private void btnHuyHoaDonTaiQuayActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnHuyHoaDonTaiQuayActionPerformed
        try {
            ...
            if (hoaDon.getTrangThai() == 3) {
                Notifications.getInstance().show(Notifications.Type.INFO, Notifications.Location.TOP_CENTER,
                        "Không thể hủy hóa đơn chờ giao hàng");
                return;
            }

            if (hoaDon.getTrangThai() == 4) {
                Notifications.getInstance().show(Notifications.Type.INFO, Notifications.Location.TOP_CENTER,
                        "Không thể hủy hóa đơn đang giao hàng");
                return;
            }

            if (hoaDon.getTrangThai() == 5) {
                Notifications.getInstance().show(Notifications.Type.INFO, Notifications.Location.TOP_CENTER,
                        "Không thể hủy hóa đơn đã giao xong");
                return;
            }

            String lydo = JOptionPane.showInputDialog("Nhập lý do hủy:");
            if (lydo == null) {
                return;
            }
            if (lydo.isEmpty()) {
    ...
    private void btnXuatDanhSachTaiQuayActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnXuatDanhSachTaiQuayActionPerformed
        ...
        String hinhThuc = (String) cbThoiGianTaiQuay.getSelectedItem();
        int confirm = JOptionPane.showConfirmDialog(this, "Bạn có muốn xuất danh sách hóa đơn " + hinhThuc + " không?");
        if (confirm == JOptionPane.YES_OPTION) {
            try {
                JFileChooser avatarChooser = new JFileChooser("F:\\");
                ...
                for (HoaDonHung h : listHoaDon) {
                    ...
                    if (h.getTrangThai() == 0) {
                        ...
                        tthd = "Chờ giao hàng";
                    } else if (h.getTrangThai() == 4) {
                        tthd = "Đang giao";
                    } else if (h.getTrangThai() == 5) {
                        tthd = "Đã giao";
                    } else if (h.getTrangThai() == 6) {
                        tthd = "Khách hẹn lại";
                    } else if (h.getTrangThai() == 7) {
                        tthd = "Đã trả hàng";
                    }

                    cell = row.createCell(7, CellType.STRING);
                    cell.setCellValue(tthd + "");
                    x++;
                    index++;
                }
                try {
    ...
    private void btnInHoaDonDatHangActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnInHoaDonDatHangActionPerformed
        try {
            ...
            if (hoaDon.getTrangThai() == 0) {
                Notifications.getInstance().show(Notifications.Type.ERROR, Notifications.Location.TOP_CENTER,
                        "Không thể in hóa đơn có trạng thái là chờ thanh toán");
                return;
            }
...
//            }
    ...
    private void btnXuatDanhSachDatHangActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnXuatDanhSachDatHangActionPerformed
        ...
        String hinhThuc = (String) cbThoiGianDatHang.getSelectedItem();
        int confirm = JOptionPane.showConfirmDialog(this, "Bạn có muốn xuất danh sách hóa đơn " + hinhThuc + " không?");
        if (confirm == JOptionPane.YES_OPTION) {
            try {
                JFileChooser avatarChooser = new JFileChooser("F:\\");
