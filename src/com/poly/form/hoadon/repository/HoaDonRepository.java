...
public class HoaDonRepository {
    ...
    public Integer countHoaDonChoThanhToan() {
        Integer result = 0;
        String query = "SELECT COUNT(IDHoaDon) FROM HoaDon WHERE TrangThai = 0 AND THoiGianTao > CONVERT(DATE, GETDATE()) ";
        try {
    ...
    public void insertKhachHangSearch(KhachHangSearch kh, String ma) {
        String query = "INSERT INTO khach_hang(ho_ten, sdt, email, dia_chi, cap_bac, is_delete, thoi_gian_tao, thoi_gian_sua, ma) VALUES(?,?,?,?,?,?,GETDATE(), GETDATE(), ?)";
        try {
            PreparedStatement ps = cn.prepareStatement(query);
...
// bắt đầu làm thêm sản phẩm vào hóa đơn
    ...
    public boolean isExitMaKhachHangSearch(String ma) {
        String query = "SELECT ma FROM khach_hang WHERE ma = ?";
        if (cn != null) {
            try {
                PreparedStatement ps = cn.prepareStatement(query);
                ps.setString(1, ma);
