...
public class FormQuanLyGiaoHangChinh extends javax.swing.JPanel {
    ...
    public void init() {
        ...
        this.tblDanhSachPhieuGiao.getTableHeader().setReorderingAllowed(false);
        ...
        try {
            ...
            JOptionPane.showMessageDialog(this, "listPhieuGiao = service lỗi");
    ...
    private PhieuGiaoHang readForm(int index, List<PhieuGiaoHang> list) throws Exception {
        int cheDo = cbxChonCheDo.getSelectedIndex();

        String ma = this.jlbMaHoaDon.getText().trim();
        String moTa = txaMoTa.getText().trim();
        String diaChi = txtDiaChiGiao.getText().trim();
        String nguoiGiao = txtNguoiGiao.getText().trim();
        String nguoiNhan = txtNguoiNhan.getText().trim();
        String sdtNguoiGiao = txtSDTNguoiGiao.getText().trim();
        ...
        if (cheDo == 0 && phieuGiaoHang.getTrangThai() != 5) {
            JOptionPane.showMessageDialog(this, "Lưu ý không được chỉnh sửa phiếu giao đã đang giao (Phải chuyển về chuẩn bị thì mới có thể chỉnh sửa)");
            return null;
        }
        ...
        if ((ngayUocTinh <= ngayHienTai && cheDo == 0)) {
            JOptionPane.showMessageDialog(this, "Ngày ước tính phải lớn hơn ngày hiện tại");
            return null;
        }

        if (diaChi == null || diaChi.trim().isEmpty() || diaChi.length() > 200) {
            JOptionPane.showMessageDialog(this, "Địa chỉ trống hoặc quá 255 ký tự");
            return null;
        }

        String regex = "^[a-zA-Z \\p{L}]+$";
        if (nguoiGiao == null || nguoiGiao.trim().isEmpty() || nguoiGiao.length() > 200) {
            JOptionPane.showMessageDialog(this, "Người giao trống hoặc quá 255 ký tự");
            return null;
        }

        if (nguoiNhan == null || nguoiNhan.trim().isEmpty() || nguoiNhan.length() > 200) {
            JOptionPane.showMessageDialog(this, "Người nhận trống hoặc quá 255 ký tự");
            return null;
        }

        if (moTa == null || nguoiNhan.trim().isEmpty() || nguoiNhan.length() > 200) {
            JOptionPane.showMessageDialog(this, "Mô tả trống hoặc quá 255 ký tự");
            return null;
        }

        if (!nguoiGiao.matches(regex)) {
            JOptionPane.showMessageDialog(this, "Tên người giao không được có ký tự số");
            return null;
        }

        if (!nguoiNhan.matches(regex)) {
            JOptionPane.showMessageDialog(this, "Tên người nhận không được có ký tự số");
            return null;
        }
            ...
            .addGroup(jPanel2Layout.createSequentialGroup()
                ...
                .addGroup(jPanel2Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    ...
                    .addGroup(jPanel2Layout.createSequentialGroup()
                        ...
                        .addComponent(cbxTimKiemPhieuGiaoTheoTrangThai, 0, 179, Short.MAX_VALUE)
                        .addGap(18, 18, 18)
    ...
    private void btnXacNhanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnXacNhanActionPerformed
        ...
        if (index >= 0) {
            if (ma == null || ma.trim().equalsIgnoreCase("")) {
                ...
                String textCombox = (String) this.cbxChonCheDo.getSelectedItem();
                int choice = JOptionPane.showConfirmDialog(this, "Chọn chế độ: " + textCombox, "Thông báo", JOptionPane.YES_NO_OPTION);
                if (choice == JOptionPane.YES_OPTION) {
                    ...
                    if(selectedIndex == 2){
                        JOptionPane.showMessageDialog(this, "Phiếu giao đã giao không thể sửa hay đổi trạng thái");
                        return;
                    }
                    
                    try {
                        if ((luaChon == 1 || luaChon == 2) && readForm(index, listPhieuGiao) == null && selectedIndex == 5) {
                            JOptionPane.showMessageDialog(this, "Chuyển phiếu giao phải điền thông tin");
                            return;
                        }
                    } catch (Exception ex) {
                        JOptionPane.showMessageDialog(this, "Phần chuyển giao đang bảo trì");
                    }
