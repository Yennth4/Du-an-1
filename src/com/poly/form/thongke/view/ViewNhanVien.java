...
//        dataset.setValue("Category 1" + 33 + "%", 30.333);
        for (NhanVienEntity Entity : nhanVienService.findAll()) {
            dataset.setValue(Entity.getTen() + " - " + Entity.getPhanTram() + "%", Entity.getPhanTram());
        }
        // Tạo biểu đồ tròn
        JFreeChart chart = ChartFactory.createPieChart(
                "Biểu Đồ Tỉ Lệ Sản Phẩm Đã Bán Theo Nhân Viên Trong 30 Ngày Gần Đây"
...
//        plot.setExplodePercent("Category 2", 0.20); // Phần bị nổ ra ,tách ra
    ...
    private void customInit(ChartPanel chartPanel) {
        ...
        jPanel1.setBorder(javax.swing.BorderFactory.createTitledBorder("Top nhân viên bán được nhiều sản phẩm nhất trong 30 ngày gần đây"));

        // Số lượng dòng trong table
        List<NhanVienEntity> list = nhanVienService.findAll();
...
// Duyệt qua danh sách và gán giá trị vào mảng
        for (int i = 0; i < rowCount; i++) {
            NhanVienEntity nhanVienEntity = list.get(i);
            data[i][0] = i + 1; // STT
            data[i][1] = nhanVienEntity.getUserName();
            data[i][2] = nhanVienEntity.getTen();
            data[i][3] = nhanVienEntity.getGioiTinh();
            data[i][4] = nhanVienEntity.getSdt();
            data[i][5] = nhanVienEntity.getSoLuongBanDuoc();
            data[i][6] = nhanVienEntity.getRole(); 
            data[i][7] = nhanVienEntity.getPhanTram() + "%";
        }
        tblList.setModel(new javax.swing.table.DefaultTableModel(
                ...
                data,
                new String[]{
                    "STT", "Mã nhân viên", "Tên", "Giới tính", "Số điện thoại", "Số lượng đã bán", "Vai trò", "Chiếm phần trăm"
                }
        ));
    ...
    private void initComponents() {
        ...
        tblList.setFont(new java.awt.Font("Segoe UI", 0, 16)); // NOI18N
        tblList.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] {
                {"1", "11", "11", "11", "11", "11", "1"},
                {null, null, null, null, null, null, null},
