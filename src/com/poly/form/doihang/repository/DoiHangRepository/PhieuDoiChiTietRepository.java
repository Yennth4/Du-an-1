...
public class PhieuDoiChiTietRepository {
    ...
    public void createPhieuDoiChiTiet(ProductDetailResponse detailResponse, Long idPD, Long idLyDoDoi, String mota, Integer soLuongDoi,String loaiDoi)
            throws Exception {
        String query = "INSERT INTO phieu_doi_chi_tiet (gia_ban, ten_san_pham, mau, so_luong_doi,id_san_pham_chi_tiet, id_phieu_doi,id_ly_do_doi,mota,loai) "
                + "VALUES (?,?,?,?,?,?,?,?,?)";
        ...
        ps.setObject(8, mota);
        ps.setObject(9, loaiDoi);
        ps.executeUpdate();
    }

    public void createPhieuDoiChiTietTienMat(ProductDetailResponse detailResponse, Long idPD, Long idLyDoDoi, String mota, Integer soLuongDoi) throws Exception {
        String query = "INSERT INTO phieu_doi_chi_tiet (gia_ban, ten_san_pham, mau, so_luong_doi,id_san_pham_chi_tiet, id_phieu_doi,id_ly_do_doi,mota) "
                + "VALUES (?,?,?,?,?,?,?,?)";
        PreparedStatement ps = conn.prepareStatement(query);
        ps.setObject(1, new BigDecimal(detailResponse.getGiaTien()).divide(new BigDecimal(detailResponse.getSoLuong())).multiply(new BigDecimal(soLuongDoi)));
        ps.setObject(2, "Tiền mặt");
        ps.setObject(3, null);
        ps.setObject(4, 1);
        ps.setObject(5, detailResponse.getIdsanphamdetail());
        ps.setObject(6, idPD);
    ...
    public Boolean checkDoiHang(Long idSPCT, Long idPD, Long idLyDo ,String loai) throws Exception {
        String query = "select * from phieu_doi_chi_tiet where id_san_pham_chi_tiet =? and id_phieu_doi =? and id_ly_do_doi =? and loai = ?";
        PreparedStatement ps = conn.prepareStatement(query);
